"""Privacy Show Any's bounded value-to-text product conversion."""
...
def _sanitize_for_text(
    ...
    if isinstance(value, Sequence) and not isinstance(
        ...
        seen.add(value_id)
        result = [
            _sanitize_for_text(item, depth + 1, seen)
...
def _is_tensor_like(value: Any) -> bool:
    ...
    return module.startswith("torch") and hasattr(value, "shape") and hasattr(
        value,
        "dtype",
    )
...
def _is_array_like(value: Any) -> bool:
    module = type(value).__module__
    return module.startswith("numpy") and hasattr(value, "shape") and hasattr(
        value,
        "dtype",
    )
...
def _truncate_text(text: str) -> str:
    if len(text) <= MAX_TEXT_CHARS:
        return text
    if _is_canonical_truncation(text):
        return text
    omitted = len(text) - MAX_TEXT_CHARS
    return f"{text[:MAX_TEXT_CHARS]}\n<truncated {omitted} character(s)>"


def _is_canonical_truncation(text: str) -> bool:
    suffix = text[MAX_TEXT_CHARS:]
    prefix = "\n<truncated "
    ending = " character(s)>"
    if not suffix.startswith(prefix) or not suffix.endswith(ending):
        return False
    omitted = suffix[len(prefix) : -len(ending)]
    return omitted.isdigit() and int(omitted) > 0
